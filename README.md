...
Manual setup: **Build command** `pip install -r requirements.txt`, **Start command** `gunicorn --bind 0.0.0.0:$PORT --worker-class eventlet -w 1 server:app`, and the same env vars.
